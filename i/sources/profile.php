	...
	elseif(!empty($resultId[1])) {
		...
		if($_GET['f'] == 'followers' || $_GET['f'] == 'following') {
			...
			if(mysql_num_rows($resultRelation) == 1) { // Verifica daca userul pe care vrea sa-l urmareasca nu e defapt el insusi
				$TMPL['follow'] = ($resultId[0] !== $data['id']) ? '<a href="'.$confUrl.'/index.php?a=profile&u='.$_GET['u'].'&r=2"><div class="follow-button">unfollow</div></a><a href="'.$confUrl.'/index.php?a=messages&pre='.$TMPL['username'].'"><div class="sendpm-button">send pm</div></a>' : '';
			} elseif(mysql_num_rows($resultRelation) == 0) {
				$TMPL['follow'] = ($resultId[0] !== $data['id']) ? '<a href="'.$confUrl.'/index.php?a=profile&u='.$_GET['u'].'&r=1"><div class="follow-button">follow</div></a><a href="'.$confUrl.'/index.php?a=messages&pre='.$TMPL['username'].'"><div class="sendpm-button">send pm</div></a>' : '';
			}
			
			$TMPL['gallery'] = '<a href="'.$confUrl.'/index.php?a=profile&u='.$_GET['u'].'&gallery=1"><div class="sendpm-button">gallery</div></a>';
			
			$profile .= $skin->make();
			$public = '_follow';
		} elseif(isset($_GET['gallery'])) {
			$TMPL_old = $TMPL; $TMPL = array();
			$skin = new skin('profile/gallery'); $rows = '';

			$data = loginCheck($_COOKIE['username'], $_COOKIE['password']);
			
			$queryMsg = sprintf("SELECT * FROM messages, users WHERE messages.uid = '%s' AND messages.uid = users.idu AND (messages.media <> '' OR messages.video <> '') ORDER BY messages.id DESC", mysql_real_escape_string($resultId[0]));
			...
			while($TMPL = mysql_fetch_assoc($resultMsg)) {
				...
				if(!empty($TMPL['media'])) {
					$TMPL['media'] = '<a href="'.$confUrl.'/uploads/media/'.$TMPL['media'].'" target="_blank"><img src="'.$confUrl.'/cache.php?src='.$TMPL['media'].'&t=m"  height="90" width="120" /></a>';
					$TMPL['type'] = 'Image';
				} else {
					if(substr($TMPL['video'], 0, 3) == 'yt:') {
						$TMPL['media'] = '<a href="http://www.youtube.com/watch?v='.str_replace('yt:', '', $TMPL['video']).'" target="_blank"><img src="http://img.youtube.com/vi/'.str_replace('yt:', '', $TMPL['video']).'/1.jpg" /></a>';
						$TMPL['type'] = 'Video';
					} else if(substr($TMPL['video'], 0, 3) == 'vm:') {
						$data = getData('http://vimeo.com/api/v2/video/'.str_replace('vm:', '', $TMPL['video']).'.json');
						$data = json_decode($data);
						$TMPL['media'] = '<a href="http://vimeo.com/'.str_replace('vm:', '', $TMPL['video']).'" target="_blank"><img src="'.$data[0]->thumbnail_small.'" height="90" width="120" /></a>';
						$TMPL['type'] = 'Video';
					}
				}
				$TMPL['delReply'] = ($TMPL['username'] == $_COOKIE['username']) 
				? 
				'<a href="'.$confUrl.'/index.php?a=me&d='.$TMPL['id'].'">Delete</a>'
				: 
				'';
				$TMPL['url'] = $confUrl;
				$newArr[] = $TMPL['id'];
			...
			$resultMsg = 1;	 // Disable the more button
			...
			if(mysql_num_rows($resultRelation) == 1) { // Verifica daca userul pe care vrea sa-l urmareasca nu e defapt el insusi
				$TMPL['follow'] = ($resultId[0] !== $data['id']) ? '<a href="'.$confUrl.'/index.php?a=profile&u='.$_GET['u'].'&r=2"><div class="follow-button">unfollow</div></a><a href="'.$confUrl.'/index.php?a=messages&pre='.$TMPL['username'].'"><div class="sendpm-button">send pm</div></a>' : '';
				if($_GET['r'] == 2) {
					$TMPL['follow'] = ($resultId[0] !== $data['id']) ? '<a href="'.$confUrl.'/index.php?a=profile&u='.$_GET['u'].'&r=1"><div class="follow-button">follow</div></a>' : '';
				...
				if($_GET['r'] == 1) {
					...
					$TMPL['follow'] = ($resultId[0] !== $data['id']) ? '<a href="'.$confUrl.'/index.php?a=profile&u='.$_GET['u'].'&r=2"><div class="follow-button">unfollow</div></a>' : '';
