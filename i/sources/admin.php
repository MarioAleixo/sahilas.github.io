	...
	if(isset($_POST['login'])) { // Set cookies for Log-in.
		...
		if(mysql_fetch_row(mysql_query($query))) { // If true - Logged-in
			...
			if($_GET['b'] == 'security') { // Security Admin Tab
				...
				for($i = 0; ($row = mysql_fetch_row($result)) !== false; $i++) {
					$img = (!empty($row[12])) ? $confUrl.'/cache.php?src='.$row[12].'&t=a' : 'http://www.gravatar.com/avatar/'.md5($TMPL['email']).'?s=50&d=mm"';
					$TMPL['users'] .= "<div class=\"admin-rows\"><div class=\"one columns\">{$row[0]}</div><div class=\"three columns\"><img src=\"".$img."\" width=\"16\" height=\"16\"/><a href=\"".$confUrl."/index.php?a=profile&u={$row[1]}\" target=\"_blank\">{$row[1]}</a></div><div class=\"seven columns\">{$row[3]}</div><div class=\"one columns\"><a href=\"".$confUrl."/index.php?a=admin&b=users&delete={$row[0]}\"><img src=\"".$confUrl."/images/icons/delete.png\" /></a></div></div>";
				}
